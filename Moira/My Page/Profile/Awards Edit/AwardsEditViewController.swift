//
//  AwardsEditViewController.swift
//  Moira
//
...
protocol AwardsEditViewControllerDelegate: class {
    func setAwardsInfo(info: AwardsInfo)
}

class AwardsEditViewController: BaseViewController {
    
    @IBOutlet weak var awardTextField: UITextField!
    @IBOutlet weak var awardBorder: UIView!
    @IBOutlet weak var dateTextField: UITextField!
    @IBOutlet weak var dateBorder: UIView!
    
    @IBOutlet weak var awardBorderHeight: NSLayoutConstraint!
    ...
    let awardsInfo: AwardsInfo?
    weak var delegate: AwardsEditViewControllerDelegate?
    
    init(info: AwardsInfo = AwardsInfo(), delegate: AwardsEditViewControllerDelegate? = nil) {
        self.awardsInfo = info
        self.delegate = delegate
        super.init(nibName: "AwardsEditViewController", bundle: nil)
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let info = awardsInfo {
            awardTextField.text = info.award
    ...
    func initTextField() {
        awardTextField.clearButtonMode = .unlessEditing
        dateTextField.clearButtonMode = .unlessEditing
        
        awardTextField.delegate = self
    ...
    @objc func editButtonClicked(sender: UIBarButtonItem) {
        if let award = awardTextField.text, let date = dateTextField.text, award != "", date != "" {
            let info = AwardsInfo(award: award, date: date)
            self.delegate?.setAwardsInfo(info: info)
...
extension AwardsEditViewController: UITextFieldDelegate {
    func textFieldDidBeginEditing(_ textField: UITextField) {
        switch textField {
        case awardTextField:
            awardBorder.backgroundColor = UIColor(hex: 0x0066FF)
            awardBorderHeight.constant = 2.5
        case dateTextField:
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        switch textField {
        case awardTextField:
            awardBorder.backgroundColor = UIColor(hex: 0xC1C1C1)
            awardBorderHeight.constant = 1
        case dateTextField:
